...
class Tools():
    ...
    def __init__(self):
        ...
        self.logger = get_logger("tools", level="INFO", handler_type="stream", filename=f"{ROOT}/logs/tools_{datetime.now().strftime('%Y_%m_%d')}.log")
    ...
    def load_exec_block(self, llm_text: str):
        blocks =  regex.findall(rf"```{self.tag}\s*(.*?)\s*```", llm_text, regex.DOTALL)
        save_path = None
        return blocks, save_path
    
if __name__ == "__main__":
    tool = Tools()
    tool.tag = "file_finder"
...
I will execute:
I’ll use file_finder to locate the file cv.md and read its contents.
